...
class TestSelfHealer:
    ...
    def heal_compile_error(
        ...
        if dependency_issue:
            ...
            self.log(" [HEAL] Cannot fix dependency issues through code modification")
            return False
        ...
        try:
            ...
            prompt = self._build_compile_error_fix_prompt(
                test_file.name, test_code, source_code_context, error_output
            )
            self.log(" [HEAL] Calling AI to analyze and fix...")
            ...
            if not fixed_code:
                self.log(" [HEAL] Failed: AI did not return valid code")
                return False

            if len(fixed_code) < 50:
                self.log(
                    f" [HEAL] Failed: Fixed code too short ({len(fixed_code)} chars)"
                )
                return False

            if "```" in fixed_code or "**" in fixed_code:
                self.log(" [HEAL] Failed: Fixed code contains markdown syntax")
                return False

            # Check for diff/patch format
            if (
                fixed_code.startswith("---")
                or fixed_code.startswith("+++")
                or fixed_code.startswith("@@")
            ):
                self.log(
                    " [HEAL] Failed: Fixed code is a diff/patch, not complete code"
                )
                return False
    ...
    def heal_test_failure(
        ...
        try:
            ...
            prompt = self._build_test_failure_fix_prompt(
                test_file.name,
                test_code,
                impl_code,
                header_code,
            ...
            self.log(" [HEAL] Calling AI to analyze test failures...")
            ...
            fixed_impl = self._extract_code_from_response(
                response, marker="// IMPLEMENTATION"
            )
            fixed_header = self._extract_code_from_response(
            ...
            if fixed_impl and impl_file:
                if len(fixed_impl) < 50 or "```" in fixed_impl or "**" in fixed_impl:
                    self.log(" [HEAL] Failed: Invalid implementation code")
                else:
                    impl_file.write_text(fixed_impl, encoding="utf-8")
                    self.log(
                        f" [HEAL] Fixed implementation written to {impl_file.name}"
                    )
                    success = True
            ...
            self.log(" [HEAL] Failed: AI did not return valid fixes")
            return False
    ...
    def _build_compile_error_fix_prompt(
        self, test_filename: str, test_code: str, source_context: str, error_output: str
    ) -> str:
        parts = []
        parts.append(
            "You are a C++ expert. A test file has compilation errors. Please fix the test code."
        )
        parts.append("")
        ...
        parts.append(
            "1. Return ONLY the complete fixed C++ code, starting with #include."
        )
        parts.append("2. Do NOT wrap the answer in markdown fences.")
        parts.append("3. Do NOT include explanations, analysis, or prose.")
        parts.append(
            "4. Preserve the test_base.h API: ASSERT_TRUE, ASSERT_EQ, RUN_TEST, TEST_MAIN_BEGIN, TEST_MAIN_END."
        )
        parts.append("5. Return compilable C++ code ONLY.")
        parts.append(
            "6. Do NOT return a diff/patch format (no ---/+++/@@ markers). Return the COMPLETE fixed file."
        )
        return "\n".join(parts)

    def _build_test_failure_fix_prompt(
        self,
        test_filename: str,
        test_code: str,
        impl_code: str,
        ...
        parts = []
        parts.append(
            "You are a C++ expert. Tests are failing and you need to fix them."
        )
        parts.append("")
        ...
        parts.append("")
        parts.append(
            "1. **Root Cause**: Why is this specific test failing? (Be specific)"
        )
        parts.append(
            "2. **Test Data Validity**: Is the test data valid according to business rules?"
        )
        parts.append(
            "   - Check: username length (3-20), password length (8-32), password complexity"
        )
        parts.append(
            "3. **Implementation Correctness**: Does the implementation match requirements?"
        )
        parts.append(
            "   - Requirements: username 3-20 chars, password 8-32 chars with letters+digits"
        )
        parts.append(
            "4. **Fix Decision**: Should we fix test code or implementation? Why?"
        )
        parts.append("")
        ...
        parts.append(
            "- Fix test data to match requirements (e.g., change 'pass123' to 'pass1234')"
        )
        parts.append(
            "- Fix implementation bugs (e.g., off-by-one errors, wrong operators)"
        )
        ...
        parts.append(
            "- Weaken security constraints (e.g., remove password complexity requirements)"
        )
        parts.append(
            "- Change business logic to make tests pass (e.g., skip password verification)"
        )
        ...
        parts.append(
            "- Infrastructure issue (missing directory/file) → Fix in implementation"
        )
        parts.append("")
        ...
        if passed == 0 and total == 0:
            parts.append(
                "**WARNING**: 0/0 means no test output - likely crash or compilation issue"
            )
        parts.append("")
        parts.append("**Test Code**:")
        parts.append("```cpp")
        ...
        parts.append("```")
        parts.append(
            test_output if test_output.strip() else "(empty - no output produced)"
        )
        parts.append("```")
        parts.append("")
        ...
        parts.append(
            "2. Return ONLY C++ code inside ```cpp blocks with // HEADER or // IMPLEMENTATION markers"
        )
        parts.append("3. Each code block must be complete and compilable")
        parts.append(
            "4. Do NOT relax validation rules unless you can justify it violates requirements"
        )
        return "\n".join(parts)

    def _build_retrieved_context_section(self, error_text: str) -> str:
        if not self.context or not bool(
            getattr(self.context, "vector_retrieval_enabled", False)
        ):
            return ""
        try:
            ...
            service.index_context(
                self.context, getattr(self.context, "project_name", "")
            )
    ...
    def _extract_code_from_response(
        self, response: str, marker: Optional[str] = None
    ) -> Optional[str]:
        # Extract and log analysis section if present
        analysis_match = re.search(r"ANALYSIS:\s*\n(.*?)(?=```|$)", response, re.DOTALL)
        if analysis_match:
            analysis = analysis_match.group(1).strip()
            self.log(" [HEAL] AI Analysis:")
            ...
            for keyword in suspicious_keywords:
                if keyword in analysis_lower:
                    self.log(
                        f" [HEAL] ⚠️  WARNING: Detected potential validation rule relaxation: '{keyword}'"
                    )
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if not self._is_valid_cpp_code(extracted):
                    self.log(
                        " [HEAL] Warning: Extracted code contains invalid C++ syntax"
                    )
                    return None
                return extracted
    ...
    def _is_valid_cpp_code(self, code: str) -> bool:
        ...
        if not has_cpp_syntax:
            self.log(" [HEAL] No C++ syntax indicators found")
            return False
        ...
        if current_file_name is not None:
            is_test_file = (
                "test_" in current_file_name.lower() or "TEST" in current_file_name
            )
            if is_test_file:
